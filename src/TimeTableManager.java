...
public class TimeTableManager {
    ...
    public TimeTableManager() {
        ...
        for (List<String> item : list) {
            ...
            TimeTable l1 = new TimeTable(item.get(0), item.get(1), item.get(2));
            timetables.add(l1);
        }
    }
    ...
    public void displayTimeTable(String roomId) {
        ...
        for (TimeTable tab : timetables) {
            if (roomId.equals(tab.getRoomId())) {
                ...
                tt[Integer.parseInt(tab.getLecture_days()) - 1][Integer.parseInt(tab.getLecture_time()) - 1] = "O";
            }
        }
        ...
        for (TimeTable tab : timetables) {
            if (roomId.equals(tab.getRoomId())) {
                if (day.equals(tab.getLecture_days())) {
                    if (lectureTime.equals(tab.showLecture_time())) {
                        return false;
                    }
                }
            }
    ...
    public void addTimeTable(String roomId, String day, String lectureTime) {// 타임테이블 추가
        TimeTable t1 = new TimeTable(roomId, day, lectureTime);
        timetables.add(t1);
    }
    ...
    public void saveDataFile() {
        ...
        for (TimeTable lec : timetables) {
            ...
            String[] tmpData = {lec.getRoomId(), lec.getLecture_days(), lec.showLecture_time(), lec.getRoomId()};
